...
`ion-trace-metric-probe` is centered on observability work in Scala. The technical target is: Package a Scala local lab for metric analysis with deny and allow fixtures, explainable decision traces, and documented operating limits.
...
The score is deterministic. The current threshold is 179. Risk has a penalty of 7, latency has a penalty of 2, and weight contributes a bonus of 6.
